...
class AccountAdvancePaymentWizard(models.TransientModel):
    _name = "rt.account.advance.payment.wizard"
    ...
    def _default_partner_id(self):
        ...
        payment = self.env['rt.account.advance.payment'].browse(active_ids)
    ...
    def advance_post_payment(self):
        ...
        context = dict(self._context or {})
        active_ids = context.get('active_ids', [])
        advance_payment = self.env['rt.account.advance.payment'].browse(active_ids)
        ...
        payment = self.env['account.payment'].create({
            ...
            'res_model': 'rt.account.advance.payment',
        ...
        # 付款金额和审核金额相等 状态改为已支付
        if float_compare(advance_payment.paid_amount, advance_payment.amount, precision_digits=0.01) == 0:
